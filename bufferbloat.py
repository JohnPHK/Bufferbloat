...
def download_webpage(net):
    h1 = net.get('h1')
    h2 = net.get('h2')
    return h2.popen('curl -o /dev/null -s -w %%{time_total} %s/http/index.html' % h1.IP()).communicate()[0]
...
def bufferbloat():
    ...
    while True:
        ...
        wait_time = download_webpage(net)
    ...
    # README and explain.
    print("The average of fetch times of webpage: %lf \n"% avg(wait_times))
    print("The standard deviation of fetch times of webpage: %lf \n"%(stdev(wait_times)))

    # Storing the fetch times for downloading a webpage in download.txt.
    with open("%s/download.txt"%(args.dir), 'w') as file_ptr:
        for t in wait_times:
            file_ptr.write("{}\n".format(t))
